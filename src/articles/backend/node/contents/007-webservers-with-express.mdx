...
Express saves developers time and empower them to work on what truly matters. It can be time-consuming to create a backend from scratch using node's `HTTP.createServer()` utilities only. Writing all the boilerplate code from port configuration, route handling etc distracts from the important task of focus on the business logic for an application.

## Create your first program

To create your first express program:

- Find an empty directory ( or folder). Directories would usually have a name so call this `"express-bac001"`
- initiate the directory as an npm module and install the express package.
  You can do `npm init -y` before running `npm i express`. But with npm v5+, `npm i express` initiates the directory before installing the express package locally to your desired folder. So can skip initiating with `npm init -y`.

<Nudge text="Global NPM package install or Local installation" />

### Paste the code below

The following program will listen on port 2999 when ran. The application
will responde to any client request on port:2999 irrespective of the HTTP
request type. If you make a HTTP POST or HTTP GET request, the response will
print "Hi, our request has been received".
...
<Nudge text="Why use CURL or POSTMAN?" />

## Routing and request handling

Routes are one of the most important building block in Express. It consist of the application request **HTTP type**, **URL** and **accompanied route handler ( typically consisdered the controller)**. When a route is matched, **one or more** handler functions for that route are called.
...
<Nudge text="Learn more about HTTP methods" />

**The HTTP type** can be HTTP GET, HTTP POST, HTTP PUT, HTTP DELETE. **The URL path** is the endpoint that should be matched. **The accompanied handler** is specified by the method as callback that is called when it detects a match from the client's request.
