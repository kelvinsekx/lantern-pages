...
This module may feel way longer compared to previously completed ones. Please don't rush it. These are the basic concepts that complex things sit on. Skipping this phase may pose problems later in the course.

Don't overly concentrate on concepts that feel difficult or confusing. We'll have numerous opportunities to discuss these concepts again along in the course.

<LessonObjectives
  contents={[
    "use console.log and understand its role in debugging",
    "read command-line arguments using process.argv",
...
`console.log` in node is a web syntatic wrapper. It is a convenience alternative to `process.stdout.write`.

Node console is a global instance configured to write to process.stdout and process.stderr. Because of this, it can be used without calling `require('node:console')`. Console.log can be used for debugging or display something to the user.
...
To explore an example, let's run using a script here.
...
To make our hands dirty, create some custom variables in a `.env` file in the root directory where you are making our sample applications.
...
Exit the REPL and restart it before running `process.env`. Try find your newly defined variables.

Oops, you wouldn't be able to find it in the output. This is because you would manually load this file if you want the variables in .env file to be included.

Start your REPL like this `node --env-file=.env`. It loads the .env file in your REPL environment.

Try find your newly defined PORTAL variable and you should see it in the list.
