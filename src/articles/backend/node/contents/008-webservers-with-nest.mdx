...
1. Install nest CLI
   ...
   On successful installation run `$ nest -v` to show the current version. If you see a number, we can assume everything is fine and nest is available globally on your computer.
...
2. Create new application
   ...
   The `nest new <project-name>` command creates a functional application with a base structure on which you can extend your implementation on.

   You'll be asked to choose a package manager. I use NPM, either Yarn or Pnpm is fine. Once the initialization is complete, you'll find instructions to start the application. I'll use `npm run start:dev` to start mine.

3. See the Live version

   Go to your browser. Visit `localhost:3000` where you should see an "Hello World!" on screen.

<LongAside subject="If port 3000 is in use, customize to a different port number">
  ```bash ERROR [NestApplication] Error: listen EADDRINUSE: address already in
  use :::3000 ``` On rare cases, you may want to customize the default port for
  reasons like an Error on starting the nest server, port:3000 been used by
  another program. Go to the `main.ts` file in the src directory. Change the
  port from 3000 to a preferred port of yours say 8686.
</LongAside>
