...
last_updated_date: "27/09/2025"
syllabus_code: "BAC002"
authors: "kelvinsekx"
---

Up until this point, we've stored information in lists, also known as arrays. Aside from the obvious inability to persist this information after a program reloads, the data was often poorly structured and redundant. We had all sorts of list anomalies. We are moving on to databases, a fascinating and long-established method of storing data.

## What is a database?

A database is an **electronically** stored collection of **structured** records with the purpose to not only store, but to organize and interact with the data. This is the modern definition of a database. Before the modern era, collections of records were stored on various media, including punch cards, paper tape, magnetic tapes and disks. Different departments in a company would design their own databases with their own copies of data.

Modern database is controlled by a database management system (DBMS). Together, the data and the DBMS, along with the applications that are associated with them, are referred to as **the database system**. DBMS solved most of the problems associated with lists and older database systems. However, they are not without their demerits, which is why lists were used in previous examples. These concerns include:
...
Note - by conventions we use singular noun when naming a table.
...
**An attribute**, also known as column or field, is an item of information which is stored about an entity. For example, the entity 'lecturer' could have attributes such as staff id, surname, forename, date of birth, telephone number, etc. An attribute can only appear in one entity, unless it is a ( foreign) key attribute in another entity.
...
One-to-One is where one occurrence of an entity relates to only one occurrence in another entity, e.g a person has one passport and a passport belong to one person.
...
A One-to-Many relationship is where one occurrence in an entity relates to many occurrences in the second entity but each occurrence in the second entity is related to at most one in the first. e.g one manager manages many employees, but each employee has only one manager.

A One-to-Many relationship is established with a foreign key.

### Many-to-Many (M:N)

A many-to-many relationship - One lecturer teaches many students and a student can be taught by many lecturers - is the relationship where many occurrences in an entity relate to many occurrences in another entity.

They are typically resolved by inserting a new entity, known as an intersection or junction table, which creates two separate One-to-Many relationships.
