...
As you progress, don't over concentrate on concepts that feel overly difficult or confusing. We'll have modules or sections where you will usually discuss these concepts.

## Console.log out

`console.log` in node is a web syntatic wrapper. It is a convenience for doing `process.stdout.write`.

Node console is a global instance configured to write to process.stdout and process.stderr. Because of this, it can be used without calling `require('node:console')`.
