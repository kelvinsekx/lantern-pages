...
last_updated_date: "01/03/2026"
syllabus_code: "BAC001"
authors: "kelvinsekx"
---

Node and the entire JavaScript ecosystem relies on **async** and **the event loop**. The mastery of these topics gives you superpowers.
...
Like in javascript, the world we live in is an indefinite event loop. The year starts by Jan. 1 ends by Dec. 31, only to begin the next day with Jan. 1. With each day capturing amazing and disastrous events.

Javascript exists in a loop, the event loop. It is an infinite loop that waits for tasks, executes them, then sleeps until it receives more tasks. It's like a worker who constantly checks for new jobs to do, completes them, and then sit waiting for the next one.

Thanks to this loop, Node can handle a lot of tasks on **a single execution thread**. Tasks run efficiently on a separate end without creating multiple threads.

## Understanding that a task can be sync or async

By default, Javascript runs in sync (not async) from top to bottom. This means when it reaches a line whose task takes a lot of time, the rest of the lines below it would wait for the heavy duty line.

In contrast, async abilities allows the Javascript engine to read the next lines without waiting for the heavy duty line to be done. While we have a single execution thread in JavaScript by default, with the async superpowers, Javascript acts like manager that direct tasks to OS/Kernel when we run async events.
...
Async is both a concept and keyword in Node ( and JavaScript).

Node associates **async** operations ( or tasks) with **events**. Whenever they are triggered, they are run independently from other non-async operations. And when they are done, it cedes control back to the execution event loop to handle.
...
<LongAside subject="Explanation with some layman examples">
  For example, **the process of sewing by one person is synchronous**. As a tailor, you can not stitch a cloth until they are cut in patterns. After cutting, you would have to weave ( depending on the material) before stitching and so on. In a nutshell, you can not go on with next process until the first is done.

But let's use another example with **a Baker, who bakes asynchronously**. They start by spreading cream while the machine is mixing the cake. If it is done to taste, you can leave what you are doing to go back to the mixed cake.
