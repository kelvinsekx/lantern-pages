...
As you progress, don't over concentrate on concepts that feel overly difficult or confusing. We'll have a separate module for concepts like this. Many you will see time and time again.

## Console.log out

`console.log` in node is a web syntatic wrapper. This is because the console module is not identically as consistent as that of browsers. It is a syntatic convenience for process.stdout.
...
<AsideNote>
  Again if process.stdout, process.stderr confuse you, don't mind it. You will
  see more of it and with lengthier explanation later.
</AsideNote>
...
In subsequently and through out this course, I will skip these terminal details and jump straight at the sample code and outputs so that the example above is written with fewer lines.
...
To explore more sophiscated examples, running node commands from REPL can be limiting. So let's do script here.
